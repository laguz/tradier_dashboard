...
def autotrade_page():
    ...
    if 'submit_put' in request.form and put_exec_form.validate_on_submit():
        ...
        payload = {
            'class': 'multileg', 'symbol': put_exec_form.underlying_symbol.data, 'type': 'credit', 'duration': 'day',
    ...
    # --- EXECUTION LOGIC FOR CALL SPREAD ---
    elif 'submit_call' in request.form and call_exec_form.validate_on_submit():
        api = get_api_for_current_user()
        if not api:
            flash('Cannot place order. Please check your API credentials.', 'danger')
            return redirect(url_for('autotrade.autotrade_page'))
        ...
        payload = {
            'class': 'multileg', 'symbol': call_exec_form.underlying_symbol.data, 'type': 'credit', 'duration': 'day',
